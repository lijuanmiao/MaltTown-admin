    ...
    <el-form class='form-item-block' :label-width="FORM_LABEL_WIDTH" :inline="true" :model="DIALOG_FORM"
      ...
      <el-row>
        <el-col :span="16">订单编号 &nbsp;&nbsp;
          ...
          <span v-if="!subId">{{data.mainOrder.orderNum}}</span> &nbsp;&nbsp;
          					<el-button type="primary" @click="showLogistics(data.subOrder)" v-permissions="'DD060103'" size="mini">查看物流</el-button>
      ...
      <el-table class='table-table orderTableBottom' :data="RESULT_DATA.resultList" :border="true" @filter-change="filterList"
        ...
        <el-table-column prop="totalDisAmt" label="总优惠" min-width="100">
          <template slot-scope="props">
            -￥{{ props.row.totalDisAmt  | currency   | currency }}
          </template>
        </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goBack() {  //上一步
        this.$router.push({path: '/order/recyclebin'})
      },
