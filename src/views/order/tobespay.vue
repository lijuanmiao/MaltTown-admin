<template>
    ...
    <el-form class='form-item-block' :label-width="FORM_LABEL_WIDTH" :inline="true" :model="DIALOG_FORM"
      ...
      <el-table style="margin-top:-17px;margin-bottom:53px"  class='table-table' :data="RESULT_DATA.resultList" :border="true" @filter-change="filterList"
        ...
        <el-table-column prop="realPayAmout" label="待付金额" min-width="100">
          <template slot-scope="props">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //分页list
      pageListOrder(page) {
        this.pageList(page, {baseUrl: 'order',callback:"getList"})
      },
      //检索
      seach() {
      ...
      showDetail(data) {
        ...
        this.$router.push({path: '/order/toPayDetail', query: query})
      },
      // 查看备注
      showRemark(data) {
        this.DIALOG_FORM = {}
