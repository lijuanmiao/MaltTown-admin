        ...
        <el-col :span="12">
          ...
          <el-row :gutter="4">
            ...
            <el-col :span="18" style="float:right;position:relative;">
              <el-col :span="24">
                <img
                  @mouseout="hideUserInfo()"
                  @mouseover="showUserInfo()"
                  :src="getHeaderUrl()"
                  class="maijiaimg"
                  style="max-width: 50px;border-radius:50%;-webkit-border-radius:50%;-moz-border-radius:50%;cursor: pointer"
                >
                <span
                  class="maijiaspan"
                  @click="showUserInfo()"
              ...
              <el-card
                ...
                v-show="userShow"
                shadow="always"
                style="position:absolute;z-index:199;pointer-events:none;"
              >
          ...
          <el-row>
            ...
            <el-col :span="18" style="float:right;line-height:30px;margin-top:-5px">
              <div style="min-width:450px; ">
                ...
                >复制</el-button><el-button type="primary" size="mini"   @click="showAddress(data.address)"  v-permissions="'DD030101'" >修改订单收货地址</el-button>
              </div>
              <el-row
                v-if="data.address.identity"
                style="line-height:20px;margin-top:0px;"
              >身份证号：&nbsp;{{data.address.identity}}</el-row>
...
<el-row class="table_top">
      ...
      <el-row style="margin-top:15px;margin-bottom:10px;">
        <el-col :span="16">
          ...
          <el-button
            type="warning"
            @click="sendOutGoods()" v-permissions="'DD010103'"
            size="mini"
          >手动发货</el-button>
       ...
       <el-table
        class="table-table orderTableBottom"
        :data="RESULT_DATA.resultList"
        :border="true"
      ...
      <el-form :model="SendGoodsForm"   :label-width="FORM_LABEL_WIDTH" :inline="true"  ref='SendGoodsForm' :rules="editForm2"  >
        <el-dialog :title="'手动发货'" :visible.sync="dialogSendGoods" :close-on-click-modal='false' width="900px">
          <el-form-item label="收货人" >
            <div >{{data.address.receiver}}/{{data.address.receivePhone}}</div>
            <div style="margin-top:-15px">{{data.address.receiveAddress}}</div>
          </el-form-item>
          ...
          <el-form-item label="快递单号" prop="courierNum" style="width:calc(100% - 20px)" >
            <el-input  v-model="SendGoodsForm.courierNum" :maxlength="30" style=""></el-input>
          </el-form-item>

          <div class="wuliu_image">
            <nav style="float:left;width:100%;margin:10px 0">选择需要放进一个物流包裹里的商品</nav>
            <el-row v-for="(good, index) in lastGoodList" :key="index" style="float:left;width:100%">
              <div style="float:left;width:calc(73% - 20px);padding:10px;background:#f4f4f4;margin-bottom:3px;position:relative">
              <img :class='{"img_on":imgOnList[index]}' :src="good.goodImg" style="width:100px;height:100px;float:left"/>
              <div style="float:left;text-align:left;margin:30px 10px;width:356px">
                {{good.goodName}}</br>
                规格&nbsp;&nbsp;{{good.goodAttrDesc}}
              </div>
              <el-button type="warning" @click="addGood(good.id,index)" v-if="!imgOnList[index]" style="right:10px;top:42px;position:absolute;z-index:100">选择</el-button>
              <span v-if="imgOnList[index]" style="color:#409eff;text-align:left;right:10px;top:42px;position:absolute;z-index:100;">
                已选择<br/>
                <u style="color:#666;cursor:pointer" @click="addGood(good.id,index)" >取消</u>
              </span>
              </div>
            </el-row>
          </div>
          <el-row justify="center" >
            <el-col :span="24"  style="margin-top:40px;text-align:center">
              ...
              <el-button type="primary" @click="saveSendGood" ><span v-if="selectedNum==lastGoodList.length">确认发货</span><span v-if="selectedNum!=lastGoodList.length">继续设置下一个模板</span></el-button>
            </el-col>
          </el-row>
        </el-dialog>
      </el-form>
          ...
          <el-col :span="18">
            <el-row>
              <el-col :span="9">
                ...
                <el-row
                  v-if="data.subOrder.crossBorder==1"
              ...
              <el-col :span="9">
                ...
                <el-row
                  style="margin-top:-10px;text-align:right"
                >￥{{data.subOrder.orderFreight | currency}}</el-row>
                <el-row
                  style="margin-top:-10px;text-align:right"
                  v-if="data.subOrder.crossBorder==1"
                >￥{{data.subOrder.totalTax | currency}}</el-row>
                <el-row style="margin-top:-10px;text-align:right" class="detail-margin">
                  <div
                    v-if="data.subOrder.crossBorder!=1"
                    style="color: darkorange;"
                  >-￥{{data.subOrder.disAmt | currency}}</div>
                  <div v-if="data.subOrder.crossBorder==1">--</div>
                </el-row>
                <el-row
                  style="color: darkorange;margin-top:-10px;text-align:right"
                >-￥{{data.subOrder.rebateAmt | currency}}</el-row>
                <el-row
                  style="color: darkorange;margin-top:-10px;text-align:right"
                >-￥{{data.subOrder.shareDis | currency}}</el-row>
                <el-row
                  style="color: darkorange;margin-top:-10px;text-align:right"
                >-￥{{data.subOrder.cash | currency}}</el-row>
              </el-col>
            ...
            <el-row style="border-top:1px dashed #ccc;text-align:right;margin-top:-20px">
              ...
              <el-col
                :span="9"
                class="bigShow_content detail-margin"
              >￥{{data.subOrder.realPayAmout| currency}}</el-col>
              <el-col :span="6">&nbsp;</el-col>
            </el-row>
          </el-col>
          <el-col :span="3">&nbsp;</el-col>
...
<script>
  ...
  export default {
    data() {
      return {
        ...baseData,
        [URL_PARAM]: { goodsList: 'controller/order/goodsList/',editRemark:'controller/order/editStoreRemark'},
        [QUERY_FORM]: {},
        [DIALOG_FORM]: {},
        editForm: { //表单验证规则
    ...
    mounted() {
      ...
      this.subId=subId
      this.getList({customUrl:this.URL_PARAM.goodsList+subId,callback:'getImageData'})
      // this.getList({customUrl:this.URL_PARAM.goodsList+subId,callback:'getImageData'})
      ...
      callBackFun() {
        switch (this.CALLBACK.callback) {
          case 'getDataList':
            ...
            break
          case 'getImageData':
            for(var i of this.CALLBACK.data.resultList){
              this.imgOnList.push(false);
            }
            break
        }
      },
    ...
    closeOrderComp(){
      ...
      goBack() {  //上一步
        ...
        if (step == 1) {
          this.$router.push({path: '/order/tobeshipped', query: {page: page}})
        }
      },
      sendOutGoods(){ //发货
        this.SendGoodsForm.subId=this.subId
        this.expressList()
        this.lastGoodList=this.RESULT_DATA.resultList
      ...
      addGood (data,index) {
        //数组下表不能直接赋值
        let type=!this.imgOnList[index];
        this.$set(this.imgOnList,index,type);
        let selectedNum=0
        this.imgOnList.forEach((item,index)=>{
          if(item)selectedNum=selectedNum+1
        })
      ...
      saveSendGood:function () {
        this.$refs.SendGoodsForm.validate((valid) => {
          if (valid) {
            // console.log("SendGoodsForm:",this.SendGoodsForm)
            this.SendGoodsForm.courierNum=this.SendGoodsForm.courierNum.trim()
            if(!this.SendGoodsForm.courierNum){
              this.$message({
                message: '请填写快递单号',
                type: 'error'
              })
            ...
            this.imgOnList.forEach((item,index)=>{
              if(item){
                selectGoods.push(this.lastGoodList[index].id)
              }else {
                lastList.push(this.lastGoodList[index])
              }
            });
            ...
            if(lastList.length >0 ){
              this.lastGoodList=lastList
            }
            let json={
              subId:this.SendGoodsForm.subId,
              expressCode:this.SendGoodsForm.expressCode,
              expressName:this.SendGoodsForm.expressName,
              courierNum:this.SendGoodsForm.courierNum,
              goodIds:selectGoods
            };
            this.selectList.push(json)
            // console.log("this.selectList:",this.selectList);

            this.SendGoodsForm.expressCode=''
            this.SendGoodsForm.expressName=''
            this.SendGoodsForm.courierNum=''
            this.imgOnList=[]
            for(var i of this.lastGoodList){
              this.imgOnList.push(false);
            }
            if(lastList.length==0){
              axios.defaults.baseURL = process.env.API_ROOT + process.env.PATH_API
              this.$http.post(`controller/order/addOrderLogistics`, this.selectList).then((result) => {
                let _result = result.data
                if (_result.resCode === SUCCESS_CODE) {
                  this.dialogSendGoods = false
                  this.$message({
                  ...
                  let errorMsg=_result.resDesc
                  this.$message({
                    message: errorMsg?errorMsg:'操作失败',
                    type: 'error'
                  })
                }
              })
      ...
      expressList() { //快递公司
        ...
        this.expressOptions = []
        axios.defaults.baseURL = process.env.API_ROOT + process.env.PATH_API
        this.$http.get('controller/order/expresslist', {params: json}).then((result) => {
          let _result = result.data
          if (_result.resCode === SUCCESS_CODE) {
      ...
      changeExpress:function (val) {
        let name = '';
        name = this.expressOptions.find((item)=>{
          if(item.value === val) {
            this.SendGoodsForm.expressName=item.label;
            return item.label;
          }//比如：选项2
        });
        // this.SendGoodsForm.expressName=name;
        // console.log("changeExpress",this.SendGoodsForm.expressName)
      },
      onCopy: function (e) {
        // console.log('复制成功！')
      },
      ...
      saveAddress(){ //保存地址
        ...
        axios.defaults.baseURL = process.env.API_ROOT + process.env.PATH_API
        this.$http.post(`controller/order/editAddress`, this[DIALOG_FORM]).then((result) => {
          let _result = result.data
          if (_result.resCode === SUCCESS_CODE) {
