  ...
  <section style="padding:10px 20px">
    <el-form class='form-item-block' :label-width="FORM_LABEL_WIDTH" :inline="true" :model="DIALOG_FORM"
        ...
        <el-col :span="12">
          ...
          <el-row :gutter="4">
            ...
            <el-col :span="18" style="float:right;position:relative;">
              <el-col :span="24">
                <img  @mouseout="hideUserInfo()" class="maijiaimg"
                  @mouseover="showUserInfo()" :src="getHeaderUrl()"
                  style="max-width: 50px;border-radius:50%;-webkit-border-radius:50%;-moz-border-radius:50%;cursor: pointer" />
                <span class="maijiaspan" @click="showUserInfo()" @mouseover="showUserInfo()" @mouseout="hideUserInfo()" style="cursor: pointer">{{data.mainOrder.userName}}<i class="el-icon-question" style="color: #E6A23C;" ></i></span>
              </el-col>
              <el-card   style="position:absolute;z-index:199;pointer-events:none;" class="box-card" :body-style="{ padding: '0px' }" v-show="userShow" shadow="always">
          ...
          <el-row>
            ...
            <el-col :span="18" style="float:right;line-height:30px;margin-top:-5px">
              <div style="min-width:450px; ">
                ...
                >复制</el-button><el-button type="primary" size="mini"   @click="showAddress(data.address)"  v-permissions="'DD010101'" >修改订单收货地址</el-button>
              </div>
              <el-row
      ...
      <el-table class="table-table orderTableBottom" :data="RESULT_DATA.resultList" :border="true" @filter-change="filterList"
        ...
        <!--<el-table-column prop="orderNum" label="订单编号" min-width="150"></el-table-column>-->
        ...
        <el-table-column prop="realPayAmout" label="待付" min-width="100">
          <template slot-scope="props">
				...
				<el-row style="width:100%">
					...
					<el-col :span="18">
						...
						<el-row style="border-top:1px dashed #ccc;text-align:right;margin-top:-20px">
							<el-col :span="9" class="bigShow_title detail-margin" style="text-align:right">待付款</el-col>
...
<script>
  ...
  export default {
    data() {
      return {
        ...baseData,
        [URL_PARAM]: { dataModelUrl:'controller/ordertopay/detail/',goodsList: 'controller/ordertopay/goodsList/',editRemark:'controller/order/editStoreRemark'},
        [QUERY_FORM]: {},
        [DIALOG_FORM]: {},
        editForm: { //表单验证规则
    ...
    methods: {
      ...
      getDataModel(subId){
        ...
        this.$http.get(this.URL_PARAM.dataModelUrl + subId, {}).then((result) => {
          let _result = result.data
          if (_result.resCode === SUCCESS_CODE) {
            let data = _result.model
            // console.info("_result",main)
            this.$set(this.stepList,'one', util.formatDate(data.mainOrder.createTime, 'yyyy-MM-dd HH:mm:ss'));
    ...
    closeOrderComp(){
      ...
      goBack() {  //上一步
        this.$router.push({path: '/order/tobepay'})
      },
      onCopy: function (e) {
        // console.log('复制成功！')
      },
