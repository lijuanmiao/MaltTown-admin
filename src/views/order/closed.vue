  ...
  <section style="padding:10px 20px">
    <el-form class='form-item-block' :label-width="FORM_LABEL_WIDTH" :inline="true" :model="DIALOG_FORM"
      ...
      <el-table style="margin-top:-17px;margin-bottom:53px"  class='table-table' :data="RESULT_DATA.resultList" :border="true" @filter-change="filterList"
        ...
        <el-table-column prop="orderNum" label="订单编号" min-width="220"></el-table-column>
        ...
        <el-table-column prop="mainOrderStatus" label="订单状态" min-width="120">
          <template scope="props">
            已关闭
            <!--<span v-if="props.row.subOrderStatus">{{ props.row.subOrderStatusDesc}}</span>-->
            <!--<span v-if="!props.row.subOrderStatus">{{ props.row.mainOrderStatusDesc}}</span>-->
          </template>
        </el-table-column>
        <el-table-column label="操作" width="130">
          <template slot-scope="scope">
            <el-button
              v-permissions="'DD050400'"
  ...
  export default {
    ...
    methods: {
      ...
      showDetail(data) {
        ...
        this.$router.push({path: '/order/closedDetail', query: query})
      },
      // 查看备注
      showRemark(data) {
        this.DIALOG_FORM = {}
