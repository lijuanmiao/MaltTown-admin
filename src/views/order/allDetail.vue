  ...
  <section style="padding:10px 20px">
				...
				<el-col :span="12">
					<el-row :gutter="4">
						...
						<el-col :span="18" style="float:right;line-height:30px">
							<div v-if="sub0.id">{{sub0.payTime | formatDate}}</div>
              <div v-if="!sub0.id">--</div>
						</el-col>
					</el-row>
					<el-row :gutter="4">
						...
						<el-col :span="18" style="float:right;line-height:30px">
							<div v-if="sub0.id">{{sub0.payPlatformDesc}}</div>
              <div v-if="!sub0.id">--</div>
						</el-col>
					</el-row>
					<el-row :gutter="4">
   ...
   <el-row class="table_top">
      ...
      <el-row style="padding:0">
        <el-col :span="12" style="padding:0;line-height:40px">
          主订单编号 &nbsp;&nbsp;{{data.mainOrder.orderNum}} &nbsp;&nbsp;

          <el-button type="primary" @click="orderComp()" size="mini">订单详情</el-button>
        </el-col>
        <el-col :span="12" style="text-align:right">下单时间：{{data.mainOrder.createTime | formatDate}}</el-col>
      </el-row>
      <div style="float:left;width:100%;background:#f0f9f9;margin-top:-5px">
      <template v-for="subItem in RESULT_DATA.resultList">
        <el-row v-if="subItem.id" style="padding:0;">
          <el-col :span="24" style="padding-left:10px;color:#666;line-height:70px">
            子订单编号 &nbsp;&nbsp;{{subItem.orderNo}} &nbsp;&nbsp;&nbsp;&nbsp;{{subItem.warehouseName}} &nbsp;&nbsp;
            <el-button v-permissions="'DD070103'" @click="showLogistics(subItem)" type="primary" size="small">查看物流
            </el-button>
          </el-col>
        </el-row>
        <el-table :style="subItem.id?'margin-top:-44px':'margin-top:0px'" class='table-table orderTableBottom' :data="subItem.goodsList" :border="true" @filter-change="filterList"
          ...
          <el-table-column label="订单状态" min-width="120">
            <template slot-scope="props">
              <span vv-if="subItem.id">
                 {{ subItem.orderStatusDesc}}
              </span>
              <span v-if="!subItem.id">
                 {{ data.mainOrder.orderStatusDesc}}
              </span>
					...
					<el-col :span="18">
						<el-row>
							<el-col :span="9">
								...
								<el-row v-if="data.mainOrder.crossBorder==1" style="margin-top:-10px;text-align:right">税费</el-row>
							...
							<el-col :span="9">
								<el-row style="text-align:right">￥{{data.mainOrder.orderAmt | currency}}</el-row>
								<el-row style="margin-top:-10px;text-align:right">￥{{data.mainOrder.freight | currency}}</el-row>
								<el-row style="margin-top:-10px;text-align:right" v-if="data.mainOrder.crossBorder==1">￥{{data.mainOrder.totalTax | currency}}</el-row>
								<el-row style="margin-top:-10px;text-align:right" class="detail-margin">
									<div v-if="data.mainOrder.crossBorder!=1" style="color: darkorange;">-￥{{data.mainOrder.disAmt | currency}}</div>
									<div v-if="data.mainOrder.crossBorder==1">--</div>
								</el-row>
								<el-row style="color: darkorange;margin-top:-10px;text-align:right">-￥{{data.mainOrder.rebateAmt | currency}}</el-row>
								<el-row style="color: darkorange;margin-top:-10px;text-align:right">-￥{{data.mainOrder.shareDis | currency}}</el-row>
								<el-row style="color: darkorange;margin-top:-10px;text-align:right">-￥{{data.mainOrder.cash | currency}}</el-row>
							</el-col>
							<el-col :span="6">&nbsp;</el-col>
						</el-row>
						<el-row style="border-top:1px dashed #ccc;text-align:right;margin-top:-20px">
							<el-col :span="9" class="bigShow_title detail-margin" style="text-align:right">
                <span v-if="data.mainOrder.orderStatus!=1">待付款</span>
                <span v-if="data.mainOrder.orderStatus==1">实付款</span>
              </el-col>
							<el-col :span="9" class="bigShow_content detail-margin">￥{{data.mainOrder.expAmt| currency}}</el-col>
							<el-col :span="6">&nbsp;</el-col>
						</el-row>
					</el-col>
					<el-col :span="3">&nbsp;</el-col>
  ...
  export default {
    data() {
      return {
        ...baseData,
        [URL_PARAM]: {
          dataModelUrl: 'controller/ordertopay/detail/',
          goodsList: 'controller/allorder/goodsList/',
    ...
    mounted() {
      ...
      this.mainId = mainId
      this.getList({customUrl: this.URL_PARAM.goodsList + mainId, callback: "getDataList"})
      this.getDataModel(mainId)
    },
    watch: {
    ...
    methods: {
      ...new method(),
      callBackFun() {
        // console.info("============callback======:",this.CALLBACK.callback)
        switch (this.CALLBACK.callback) {
          case 'getDataList':
            if (this[RESULT_DATA].resultList.length > 0) {
              let sub0 = this[RESULT_DATA].resultList[0]
              this.sub0 = sub0
            }
            break
        }
      ...
      getDataModel(mainId) {
        ...
        this.$http.get(this.URL_PARAM.dataModelUrl + mainId, {}).then((result) => {
      ...
      saveRemark() { //保存备注
        ...
        this.$http.post(this.URL_PARAM.editRemark, this.StoreRemarkForm).then((result) => {
          ...
          if (_result.resCode === SUCCESS_CODE) {
            ...
            this.getList({customUrl: this.URL_PARAM.goodsList + this.mainId})
          } else {
            this.$message({
              message: '操作失败',
              type: 'error'
