<template xmlns:v-clipboard="http://www.w3.org/1999/xhtml">
  <section style="padding:10px 20px">
    <el-form class='form-item-block' :label-width="FORM_LABEL_WIDTH" :inline="true" :model="DIALOG_FORM"
				...
				<el-col :span="12">
					<el-row :gutter="4">
						...
						<el-col :span="18" style="float:right;line-height:30px">
							<div v-if="data.subOrder.payTime">{{data.subOrder.payTime | formatDate}}</div>
              <div v-else>--</div>
						</el-col>
					</el-row>
					<el-row :gutter="4">
						...
						<el-col :span="18" style="float:right;line-height:30px">
							<div>{{data.subOrder.payPlatformDesc?data.subOrder.payPlatformDesc:'--'}}</div>
						</el-col>
					</el-row>
					<el-row :gutter="4">
						<el-col :span="5" class="title_info" style="padding-left:10px;font-weight:normal">
      ...
      <el-row>
        <el-col :span="16">订单编号 &nbsp;&nbsp;
          <span v-if="subId">{{data.subOrder.orderNo}} &nbsp;&nbsp;{{data.subOrder.warehouseName}} &nbsp;&nbsp;</span>
          <span v-if="!subId">{{data.mainOrder.orderNum}}</span> &nbsp;&nbsp;
          	<el-button type="primary" @click="showLogistics(data.subOrder)" v-permissions="'DD050103'" size="mini">查看物流</el-button>
          <el-button type="primary" @click="orderComp()" size="mini">订单详情</el-button>
        </el-col>
        <el-col :span="8" style="text-align:right">下单时间：{{data.mainOrder.createTime | formatDate}}</el-col>
      </el-row>
      ...
      <el-table class='table-table orderTableBottom' :data="RESULT_DATA.resultList" :border="true" @filter-change="filterList"
        ...
        <el-table-column label="订单状态" min-width="120">
          <template slot-scope="props">
            已关闭
            <!--<span v-if="subId">{{ data.subOrder.orderStatusDesc}} </span>-->
            <!--<span v-if="!subId">{{ data.mainOrder.orderStatusDesc}} </span>-->
          </template>
        </el-table-column>
        <el-table-column label="操作" width="130">
          <template slot-scope="scope">
            <el-button
              v-permissions="'DD050102'"
...
<el-dialog :title="'订单详情'" :visible.sync="dialogOrderComp">
              ...
              <el-col :span="9"  v-if="!subId">
						...
						<el-row style="border-top:1px dashed #ccc;text-align:right;margin-top:-20px">
							...
							<el-col :span="9" class="bigShow_content detail-margin"  v-if="subId">￥{{data.subOrder.realPayAmout| currency}}</el-col>
              <el-col :span="9" class="bigShow_content detail-margin"  v-if="!subId">￥{{data.mainOrder.expAmt| currency}}</el-col>
							<el-col :span="6">&nbsp;</el-col>
						</el-row>
					</el-col>
					<el-col :span="3">&nbsp;</el-col>
...
<script>
  ...
  export default {
    data() {
      return {
        ...baseData,
        [URL_PARAM]: { getDataModelUrl:'controller/orderclosed/detail/',goodsList: 'controller/ordertopay/goodsList/',subgoodsList: 'controller/order/goodsList/',editRemark:'controller/order/editStoreRemark',
    ...
    methods: {
      ...
      getGoodsList(){
        if(this.subId) {
          this.getList({customUrl:this.URL_PARAM.subgoodsList+this.subId})
        }else{
      ...
      getDataModel(){
        ...
        let json={'subId':this.subId}
        this.$http.get(this.URL_PARAM.getDataModelUrl + this.mainId,  {params: json}).then((result) => {
          let _result = result.data
          if (_result.resCode === SUCCESS_CODE) {
            let data = _result.model
            if(data.subOrder==null) data.subOrder={}
            this.data = data
      ...
      goBack() {  //上一步
        this.$router.push({path: '/order/closed'})
      },
