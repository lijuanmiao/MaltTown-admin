...
<el-dialog :title="'订单详情'" :visible.sync="dialogOrderComp">
      ...
      <el-dialog title="物流详情" :visible.sync="dialogShowLogistics" :close-on-click-modal='false'>
        <el-alert v-if="logisticsList &&logisticsList.length>1" type="warning" :closable="false">
          {{logisticsList.length+'个包裹已发出'}}
        </el-alert>
        <div v-if="logisticsList &&logisticsList.length>1">
          <template v-for="item in logisticsList">
            <div class="wuliu_box">
              <div class="wuliu_number"><span><i class="el-icon-tickets"></i>{{item.expressStatus}}</span><span>{{item.expressName}}&nbsp;{{item.courierNum}}  </span>
              </div>
              ...
              <div class="wuliu_number"><span>共 {{item.goodsList.length}} 件商品</span>
                <el-button @click="showLogisticsDetail(item)" type="primary" size="small"><span>查看物流信息</span>
                </el-button>
              </div>
            </div>
          </template>
        ...
        <div style="min-height: 200px;">
          <el-steps direction="vertical" :active="LogisticsDetail.expressList.length">
            <template v-for="item in LogisticsDetail.expressList">
              <el-step :title="item.context?item.context:''" :description="item.time" icon="el-icon-info"></el-step>
            </template>
          </el-steps>
        </div>
