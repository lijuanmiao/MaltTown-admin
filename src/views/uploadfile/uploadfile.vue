<template>
  <section style="padding:10px 20px">
    ...
    <el-form class='form-item-block'  :label-width="FORM_LABEL_WIDTH" :inline="true" :model="DIALOG_FORM" ref='DIALOG_FORM' :rules="editForm">
        <el-form-item label="图片名称:" prop="imgDesc">
                  <el-input v-model="DIALOG_FORM.imgDesc" :maxlength="20"></el-input>
                </el-form-item>
        <el-form-item label="首页分享图片:" class='required upload'>
                  <el-upload
                    class="upload-demo"
                    :action="fileUrl"
                    ...
                    <el-button :class="fileList.length > 0 ? 'upload-dis-btn' : ''" size="small" type="primary"
                               :disabled='fileList.length>0' v-permissions="'SC010101'">点击上传
                    </el-button>
                    <div slot="tip" class="el-upload__tip" style='line-height:15px'>建议尺寸:540*540px，只能上传jpg/png格式文件，文件不能超过1MB</div>
                  </el-upload>
                </el-form-item>
...
<script>
  ...
  export default {
    data() {
      return {
        ...baseData,
        [URL_PARAM]: { saveImg: 'my/edit/password',},
        [QUERY_FORM]: {},
        editForm: {
